...
int e1000_transmit(struct mbuf *m)
{
  ...
  // printf("txidx: %d\n", tx_idx);
  if (tx_idx < 0 || tx_idx >= RX_RING_SIZE)
  {
    release(&e1000_lock);
    return -1;
  }
...
e1000_recv(void)
{
  ...
  for (;;)
  {
    int rx_idx = (regs[E1000_RDT] + 1) % RX_RING_SIZE;
    // printf("rxidx: %d\n", rx_idx);
    ...
    if (!newm)
      panic("e1000");
    rx_mbufs[rx_idx] = newm;
    memset(&rx_ring[rx_idx], 0, sizeof(struct rx_desc));
    rx_ring[rx_idx].addr = (uint64)newm->head;
